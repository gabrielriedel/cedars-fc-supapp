...
export async function POST(req: NextRequest) {
    ...
    const { activityName, hour, day, session, year, program} = await req.json();

    if (!activityName ||  !hour || !day || !session || !year || !program) {
    ...
    const { data, error } = await supabase
        ...
        .insert([
            { activity_name: activityName, hour:hour, day:day, session:session, year: year, program:program}
        ]);
